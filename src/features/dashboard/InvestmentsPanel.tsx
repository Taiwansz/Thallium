  ...
  return (
    <div className="space-y-6 max-w-5xl">
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-bold tracking-tight text-zinc-100 font-sans">Mesa de Investimentos Thallium</h2>
        ...
        {/* Portfolio Balance */}
        <Card className="p-4 bg-zinc-950 flex flex-col justify-center border-zinc-850">
          <span className="text-[10px] text-zinc-500 font-mono uppercase tracking-wider">CARTEIRA CONSOLIDADA</span>
          <span className="text-xl font-bold font-mono text-primary tabular-nums mt-0.5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-start">
        ...
        <Card className="md:col-span-1">
          <CardHeader>
            <CardTitle className="text-sm font-semibold uppercase tracking-wider text-zinc-400">Nova Aplicação</CardTitle>
            <CardDescription>Escolha o título e o valor a aplicar.</CardDescription>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleApply} className="space-y-4">
              <div className="flex flex-col space-y-1.5 w-full">
                <label className="text-xs font-semibold uppercase tracking-wider text-zinc-400">Tipo de Título</label>
                <select
                  value={invType}
        ...
        <Card className="md:col-span-2">
          <CardHeader>
            <CardTitle className="text-sm font-semibold uppercase tracking-wider text-zinc-400">Posições Ativas</CardTitle>
            <CardDescription>Acompanhe o rendimento de suas aplicações.</CardDescription>
          </CardHeader>
          <CardContent className="p-0">
            <div className="divide-y divide-zinc-800">
              {isLoading ? (
                <div className="p-6 text-center text-sm text-zinc-400 animate-pulse">Carregando posições...</div>
              ) : !investments || investments.length === 0 ? (
                <div className="p-6 text-center text-sm text-zinc-500 font-sans">Nenhum investimento ativo neste perfil contábil.</div>
              ) : (
                investments.map((inv) => {
                  const values = calculateCurrentValue(inv);
                  return (
                    <div key={inv.id} className="p-4 flex items-center justify-between hover:bg-zinc-850/20 transition-colors">
                      ...
                      <div className="flex items-center space-x-6">
                        ...
                        <Button
                          variant="secondary"
                          className="py-1.5 px-3 text-xs border-zinc-800 cursor-pointer"
